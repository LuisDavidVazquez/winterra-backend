...
import { RemoveBestFriendDTO, FriendshipResponseDTO } from '../dtos/FriendshipDTO';

export class RemoveBestFriendUseCase {
  constructor(private friendshipRepository: IFriendshipRepository) {}

  async execute(data: RemoveBestFriendDTO): Promise<FriendshipResponseDTO> {
    ...
    if (!friendship.isAccepted()) {
      throw new Error('Cannot remove best friend. Friendship must be accepted first');
    }

    // Remove best friend status
    friendship.removeAsBestFriend();
