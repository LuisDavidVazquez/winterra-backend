...
import { AcceptDuelDTO, HabitDuelResponseDTO } from '../dtos/HabitDuelDTO';

export class AcceptHabitDuelUseCase {
  constructor(private habitDuelRepository: HabitDuelRepository) {}

  async execute(data: AcceptDuelDTO): Promise<HabitDuelResponseDTO> {
    ...
    // Verificar que el usuario que acepta es el opponent
    if (duel.getOpponentId() !== data.opponentId) {
      throw new Error('Only the opponent can accept the duel');
    }
